...
python3 basketball.py #超音波感測器，感測球有沒有投進，直到五顆球都進，同時，OLED 會不斷顯示投進球數，直到投籃環節完成。同時，Speaker 會繼續播放很躁的音樂(或昨天錄出來的美音)，當球投進的時候，會有華麗的進球音樂，再說「早安，今天也要乖乖起床喔，請投入幾顆球」，在繼續播放音樂 #import ultrasonic #import oled #aplay
...
while [ $correct -ne 1 ]; do
    ...
    python3 mi2s_asr_exp.py --num=3 > my_ans.txt #並且丟給 speech to text(asr) 
    cat my_ans.txt
    correct=$((python3 check_answer.py) 2>&1) #然後檢查答案是否正確(正確率 50%即可)
    echo "$correct" 
done
